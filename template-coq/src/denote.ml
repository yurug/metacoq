  ...
  let rec aux evm (trm: Constr.t) : _ * Constr.t =
    ...
    | ACoq_tConst (s,u) ->
       ...
       (try
          ...
          | Globnames.ConstRef c -> evm, Constr.mkRef (Globnames.ConstRef c, u)
    ...
    | ACoq_tProj (proj,t) ->
       ...
       let narg = unquote_nat arg in
       let projs = Environ.get_projections (Global.env ()) ind' in
       (match projs with
        | Some projs ->
          let evm, t' = aux evm t in
          evm, Constr.mkProj (Names.Projection.make projs.(narg) false, t')
        | None -> bad_term trm)
...
let denote_local_decl evm trm =
  let (h,args) = app_full trm [] in
  match args with
    na :: b :: ty :: [] ->
    let b = denote_option b in
    let na = unquote_name na in
    let evm, ty = denote_term evm ty in
    (match b with
     | None ->
       evm, Context.Rel.Declaration.LocalAssum (na, ty)
     | Some b ->
       let evm, b = denote_term evm b in
       evm, Context.Rel.Declaration.LocalDef (na, b, ty))
  | _ -> bad_term_verb trm "denote_local_decl"
...
let denote_variances evm l (* of type option list variance *) : _ * Variance.t array =
  evm, Array.of_list (List.map denote_variance l)
  (* FIXME let variances = List.map denote_variance (unquote_list variances) in *)
...
let unquote_mutual_inductive_entry evm trm (* of type mutual_inductive_entry *) : _ * Entries.mutual_inductive_entry =
  ...
  if Constr.equal h tBuild_mutual_inductive_entry then
    ...
    | record::finite::params::inds::univs::variance::priv::[] ->
       ...
       let evm, variance =
         match variance with
         | Some l -> let evm, l' = denote_variances evm (unquote_list l) in
           evm, Some l'
         | None -> evm, None
       in
...
let rec run_template_program_rec ?(poly = false) ?(intactic=false) (k : Environ.env * Evd.evar_map * Constr.t -> unit) env ((evm, pgm) : Evd.evar_map * Constr.t) : unit =
  ...
  | TmDefinition (name,s,typ,body) ->
    ...
    let univs =
      if poly then Polymorphic_entry ([||], Evd.to_universe_context evm)
      else Monomorphic_entry (Evd.universe_context_set evm) in
  ...
  | TmAxiom (name,s,typ) ->
    let name = reduce_all env evm name in
    let evm, typ = (match denote_option s with Some s -> let red = unquote_reduction_strategy env evm s in reduce env evm red typ | None -> evm, typ) in
    let param = Entries.ParameterEntry (None, (typ, Monomorphic_entry (Evd.universe_context_set evm)), None) in
  ...
  | TmLemma (name,s,typ) ->
    ...
    Obligations.check_evars env evm;
       ...
       ignore (Obligations.add_definition ident ~term:c cty ctx ~kind ~univ_hook:hook obls)
  ...
  | TmMkDefinition (name, body) ->
    ...
    let _ = Declare.declare_definition ~kind:Decl_kinds.Definition (unquote_ident name) (trm, Monomorphic_entry (Evd.universe_context_set evm)) in
    let env = Global.env () in
    k (env, evm, unit_tt)
  | TmQuote trm ->
